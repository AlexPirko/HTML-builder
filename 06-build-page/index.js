...
fs.mkdir(path.join(__dirname, "project-dist"), { recursive: false },
  function () {}
);

function buildAssets(origDir, buildDir){
    fs.mkdir(buildDir, { recursive: false },
      (err) => {
        if (err) console.log(err);        
          fs.readdir(path.join(origDir), { withFileTypes: true },
            (err, files) => {
              if (err) console.log(err);
              else {
                files.forEach((file) => {
...
function buildHtml(files = [], dataStr) {
    if (!files.length) { fs.writeFile(path.join(__dirname, "project-dist", "index.html"), dataStr,
        function (err) {
            if (err) throw err; 
        });
    } else {
...
function buildCss(files = [], dataStr) {
  if (!files.length) {
    dataStr.end();
        ...
        createStream.pipe(dataStr, { end: false });
